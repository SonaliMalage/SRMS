<?php
 ...
 //$result1 = mysqli_query($conn,"select astu.name, astu.rollno from adminstudent astu where and astu.email='".$_SESSION['email']."'");
        ...
        $result2 = mysqli_query($conn, "select * from adminteacher where email='".$_SESSION['email']."'");
		$row2 = mysqli_fetch_assoc($result2);

        
        //change status of all messages to read
         mysqli_query($conn, "UPDATE received_messages SET notificationStatus=1 WHERE receiver = '".$row2['ID']." - ".$row2['name']."';");
...
<body style="background-image: url('assets/img/bgn.jpg');background-size: cover;">
    ...
    <div id="messagelist">
        <center><span style="color: gray;align-self: ">When you receive messages they will appear here...</span></center><br>
        <div id="message">
            
            <?php
                $sqlQuery = "SELECT * FROM received_messages WHERE receiver = '".$row2['ID']." - ".$row2['name']."'  ORDER BY date DESC ,time DESC";
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    while($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo "<textarea name='msg' placeholder='Reply' required></textarea><br>";
                        echo "<br><input type='submit' name='submit' class='btn btn-primary' style='color: white;border-radius: 5px; border: none; box-shadow: 0px 8px 15px rgba(0, 0, 0, 0.2);background-color: #6665b1;width:120px;' value='Send'>";
                        echo "<br><br>"."</form>";
                        if($_SERVER["REQUEST_METHOD"] == "POST"){
                    $receiver=mysqli_real_escape_string($conn, $row["sender"]);
                    $sender= $row2['ID']." - ".$row2['name'];
                    $DATE=date("Y/m/d"); 
                    $time=date("h:i A");
                ...
                if($row['msgid']==$_POST['ids'])
                    ...
                    if(isset($run)){
                        ...
                        echo "<script>window.location.href='terecmsg.php';</script>";    
                     } 
                       } }

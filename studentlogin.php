...
if(isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true){
  header("location: studentsemesters.php");
  exit;
}
...
if(count($_POST)>0) {
   // $conn = mysqli_connect("localhost","root","","sturms");
     ...
     $result = mysqli_query($conn,"SELECT * FROM adminstudent WHERE password = '".$password1."'and email= '". $_POST["email"]."';")or die('alert'.$password1);
...
$rowstu = mysqli_fetch_assoc($result);
    ...
    if($count==0) {
        ...
        echo "<script>window.location.href='./studentsemesters.php';</script>";
...
<body style="background-image: url('assets/img/bgn.jpg');background-size: cover;">
    ...
    <div class="login-clean">
        <form class="shadow" method="post" style="max-width: 365px;">
            ...
            <div class="form-group"><input class="form-control" type="text" required="" name="email" placeholder="Email" autofocus=""></div>
            <div class="form-group"><input class="form-control" type="password" name="password" required="" placeholder="Password"></div>
            <div class="form-group"><input type="submit" name="login" value="Log In" class="btn btn-primary btn-block" style="background-color: #6665b1;"></div><a class="forgot" href="studentforpass.php">Forgot your password?</a>
            </form>
...
$queryl="insert into logindetails values ('','$email','$password1');";
                    ...
                    if(isset($runl)){
                        //session_start(); 
                        $_SESSION['name']=$rowstu['name'];
                        $_SESSION['rollno']=$rowstu['rollno'];
                         $_SESSION['roll']=$rowstu['ids'];

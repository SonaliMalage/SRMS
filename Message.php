...
<div class="login-clean" >
        <form class="shadow" method="post" style="max-width: 450px">
            <h2 class="text-center">Contact To Student</h2>
            <br>
            <div class="form-group">
             <select class="form-control" name="sturollno" style="color: rgb(0,0,0);">
                <option value="">Student</option>
             <?php 
             $sqlid = "SELECT astud.rollno ,astud.name from adminstudent astud, adminteacher ate, adminsemester s, adminstusem adst, adminsubjects adsb where adst.rollno = astud.ids and s.id=adsb.semid and adst.sem= adsb.semid and adsb.teacherid=ate.idt and ate.idt ='".$_SESSION['idt']."'";
             $resultid = mysqli_query($conn,$sqlid); 
               
            while ($row3 = mysqli_fetch_array($resultid)) {
            echo "<option value='" . $row3['rollno']." - ".$row3['name'] . "'>" . $row3['rollno'] ." (". $row3['name'].")"."</option>";
        } 
             ?>
            </select></div>
...
<?php
                ...
                if($_SERVER["REQUEST_METHOD"] == "POST"){
                    ...
                    $receiver=mysqli_real_escape_string($conn, $_POST["sturollno"]);
                    $sender= $_SESSION['ID']." - ".$_SESSION['name'];
